...
public class BookingsRepository implements BookingsRepositoryInterface {
	...
	private final static String SELECT_ONE_BOOKING_BY_TRAVEL_REQUEST_ID = "select * from BOOKINGS join TRAVEL_REQUESTS using (TRAVEL_REQUEST_ID) join EMPLOYEES using (employee_id) join SLAB using (slab_id) where TRAVEL_REQUEST_ID=?";
	...
	public boolean addNewBooking(Bookings bookings) {
		...
		Object[] parameters = { bookings.getTravelRequests().getTravelRequestId(), bookings.getHotelName(),
				bookings.getHotelLocation(), bookings.getCheckInTime(), bookings.getCheckOutTime(),
				bookings.getFlightTicket(), bookings.getBusTicket(), bookings.getTrainPnr()
//				bookings.getTicket(),
//				bookings.getForex()
	...
	public Bookings updateBooking(Bookings bookings) {
		Object[] parameters = { bookings.getHotelName(), bookings.getHotelLocation(), bookings.getCheckInTime(),
				bookings.getCheckOutTime(), bookings.getBusTicket(), bookings.getTrainPnr(), bookings.getFlightTicket(),
				bookings.getTicket(),
//				bookings.getForex(),
//				bookings.getTravel_Requests(),
	...
	public boolean insertFile(Bookings bookings2) {
		...
		Object[] parameters = { bookings2.getTicket(), bookings2.getTravelRequests().getTravelRequestId() };
	...
	public Bookings getBookingByTravelRequestId(int travelRequestId) {
		BookingsRowMapper bookingsRowMapper = new BookingsRowMapper();
		return jdbcTemplate.queryForObject(SELECT_ONE_BOOKING_BY_TRAVEL_REQUEST_ID, bookingsRowMapper, travelRequestId);
	}
